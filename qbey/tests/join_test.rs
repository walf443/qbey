...
// ── JoinCondition::Expr with binds ──

#[test]
fn test_join_condition_expr_with_binds() {
    let mut q = qbey("users");
    q.join(
        "roles",
        join::on_expr(
            RawSql::new(r#""users"."role_id" = "roles"."id" AND "roles"."active" = {}"#)
                .binds(&[true]),
        ),
    );
    q.select(&["id", "name"]);
    ...
    assert_eq!(
        sql,
        r#"SELECT "id", "name" FROM "users" INNER JOIN "roles" ON "users"."role_id" = "roles"."id" AND "roles"."active" = ? WHERE "id" = ?"#
    );
    assert_eq!(binds, vec![Value::Bool(true), Value::Int(1)]);
}

#[test]
fn test_join_condition_expr_with_binds_pg() {
    let mut q = qbey("users");
    q.join(
        "roles",
        join::on_expr(
            RawSql::new(r#""users"."role_id" = "roles"."id" AND "roles"."type" = {}"#)
                .binds(&["admin"]),
        ),
    );
    q.select(&["id"]);
    ...
    assert_eq!(
        sql,
        r#"SELECT "id" FROM "users" INNER JOIN "roles" ON "users"."role_id" = "roles"."id" AND "roles"."type" = $1 WHERE "active" = $2"#
    );
    assert_eq!(
        binds,
        vec![Value::String("admin".to_string()), Value::Bool(true)]
    );
}
