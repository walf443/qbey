...
impl<V: Clone + std::fmt::Debug> Query<V> {
    ...
    pub fn to_sql_with(&self, dialect: &dyn Dialect) -> (String, Vec<V>) {
        ...
        StandardSqlRenderer.render_select(&tree, &RenderConfig { ph: &ph, qi: &qi, backslash_escape: dialect.backslash_escape() })
    }
    ...
    pub fn to_pipe_sql_with(&self, dialect: &dyn Dialect) -> (String, Vec<V>) {
        ...
        let ph = |n: usize| dialect.placeholder(n);
        let qi = |name: &str| dialect.quote_identifier(name);
        PipeSqlRenderer.render_select(&tree, &RenderConfig { ph: &ph, qi: &qi, backslash_escape: dialect.backslash_escape() })
    }
}
...
impl<V: Clone + std::fmt::Debug> UnionQuery<V> {
    ...
    pub fn to_sql_with(&self, dialect: &dyn Dialect) -> (String, Vec<V>) {
        ...
        let ph = |n: usize| dialect.placeholder(n);
        let qi = |name: &str| dialect.quote_identifier(name);
        StandardSqlRenderer.render_union(&tree, &RenderConfig { ph: &ph, qi: &qi, backslash_escape: dialect.backslash_escape() })
    }
    ...
    pub fn to_pipe_sql_with(&self, dialect: &dyn Dialect) -> (String, Vec<V>) {
        ...
        let ph = |n: usize| dialect.placeholder(n);
        let qi = |name: &str| dialect.quote_identifier(name);
        PipeSqlRenderer.render_union(&tree, &RenderConfig { ph: &ph, qi: &qi, backslash_escape: dialect.backslash_escape() })
    }
